...
## 4. Quy Tắc Phát Triển & Vận Hành 📐

### 4.1. Coding Standards

| Quy tắc | Mô tả |
|---|---|
| **TypeScript strict** | Toàn bộ codebase type-safe, không dùng `any` trừ trường hợp buộc |
| **Zod validation** | Schema validation cho form + API (shared) |
| **Feature module pattern** | Mỗi tính năng = 1 thư mục: `page.tsx`, `components/`, `hooks/`, `lib/`, `types/` |
| **Shared components** | Tái sử dụng: `InputNumber`, `UploadImage`, `ExportDialog`, `ImportDialog`, v.v. |
| **apiFetch wrapper** | Tập trung xử lý 401/403, không dùng `fetch` trực tiếp |
| **Golden Rule** | "Trước khi viết mới — kiểm tra đã tồn tại chưa" |

### 4.2. Data Integrity

| Biện pháp | Chi tiết |
|---|---|
| **Referential Integrity** | Prisma `@relation` với `onDelete: Cascade` — xóa cascade tự động |
| **Unique Constraints** | `@@unique` trên các bộ field logic (VD: `[cuaHangId, maDonHang]`) |
| **Generated Codes** | `generateCode('KH')` → mã unique theo thời gian: `KH260302...` |
| **Audit trail** | Mọi thay đổi được ghi nhật ký, hỗ trợ rollback/tra cứu |

### 4.3. Health Monitoring

| Endpoint | Tác dụng |
|---|---|
| `/api/health` | Health check endpoint — kiểm tra DB connection, uptime |
...
## 6. Khuyến Nghị Nâng Cấp (Roadmap)

| Ưu tiên | Hạng mục | Hiện tại | Nâng cấp |
|---|---|---|---|
| 🔴 Cao | Rate Limiting | In-memory (single instance) | → Redis (Upstash) cho multi-instance |
| 🔴 Cao | Kiểu dữ liệu tiền tệ | `Float` | → `Decimal` (tránh lỗi làm tròn) |
| 🟡 Trung bình | Caching | Chỉ client-side (TanStack Query) | → Redis cache cho static data (danh mục, cửa hàng) |
| 🟡 Trung bình | CSP Headers | Chưa có | → Content Security Policy header |
| 🟢 Thấp | Connection Pooling | Prisma built-in | → PgBouncer / Prisma Accelerate cho high traffic |
| 🟢 Thấp | monitoring | Health endpoint cơ bản | → APM (Sentry, Datadog) |
